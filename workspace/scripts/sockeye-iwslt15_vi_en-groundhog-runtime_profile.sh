...
mkdir -p ${SOCKEYE_ROOT}/workspace/results/profile

cd ${SOCKEYE_ROOT} && rm -rf ${SOCKEYE_ROOT}/workspace/${CONFERENCE_SRC_TGT} && \
PYTHONPATH=${SOCKEYE_ROOT} \
/usr/local/cuda/bin/nvprof --profile-from-start off \
